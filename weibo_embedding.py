...
Sentiment analysis using SnowNLP and Textblob for potential labels in the dataset
'''
...
def new_corpus(tweet_list):
    ...
    # save the lookup
    new_vocab.to_csv('new_vocab_weibo.csv',header = True, index = False)
...
# read data
tweet_df = pd.read_json('1.json')

# process text 
# emoji
tweet_df['process_1'] = tweet_df['content'].apply(preprocess_tweet)
tweet_df['process_2'] = tweet_df['process_1'].apply(html_symbol)

# tokenize
tweet_df['token'] = tokenize_tweet(tweet_df['process_2'].tolist())

# sequence length
tweet_df['seq_length'] = tweet_df['token'].apply(sequence_length)

# use reduced sample
tweet_sample_df = tweet_df.loc[tweet_df['seq_length']==126].copy()
...
with open('weibo_seq_10.txt', 'w') as f:
    for item in tweet_list:
        item = str(item[1:11]).replace(',','').replace('[','').replace(']','')
        f.write("%s\n" % item)

tweet_sample_df['reduced_token'] = tweet_sample_df['token']

test_tweet_encoded = tokenizer(tweet_list.tolist(),padding=True, truncation=True)
